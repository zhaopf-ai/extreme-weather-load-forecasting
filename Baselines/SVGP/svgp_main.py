...
class TrainConfig:
    ...
    kernel_lengthscale: float = 4.0
    num_inducing: int = 200

    random_state: int = 42
    tf_seed: int = 42
    gpflow_default_float: str = "float64"
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--save_root", type=str, default=r"D:\python project\image extreme weather\svgp_results")
    ...
    p.add_argument("--kernel_lengthscale", type=float, default=4.0)
    p.add_argument("--num_inducing", type=int, default=200)

    p.add_argument("--random_state", type=int, default=42)
    p.add_argument("--tf_seed", type=int, default=42)
    p.add_argument("--gpflow_default_float", type=str, default="float64", choices=["float32", "float64"])
...
def make_config(args: argparse.Namespace) -> TrainConfig:
    return TrainConfig(
        ...
        gpflow_default_float=args.gpflow_default_float,
    )
...
def build_svgp_model(x_train: np.ndarray, cfg: TrainConfig) -> gpflow.models.SVGP:
    m = min(cfg.num_inducing, x_train.shape[0])
    inducing_variable = x_train[:m, :].copy()

    kernel = gpflow.kernels.SquaredExponential(lengthscales=cfg.kernel_lengthscale)
...
def fit_svgp_model(model: gpflow.models.SVGP, x_train: np.ndarray, y_train: np.ndarray):
...
def predict_svgp_model(model: gpflow.models.SVGP, x_test: np.ndarray):
...
def main():
    ...
    if cfg.gpflow_default_float == "float32":
